...
namespace Contacts.CommandResultParsers
{
    public class GetConcactsCommandResultParser : IResultCommandParser
    {
        public Result Parse(string result)
        {
            ...
            var values = Enum.GetValues(typeof(ContactColumnEnum))
        ...
        private string GetValue(ContactColumnEnum value, string row)
        {
            ...
            if (Enum.IsDefined(typeof(ContactColumnEnum), nextEnum))
            {
                var nextDesc = ((ContactColumnEnum)nextEnum).AsString(EnumFormat.Description);
